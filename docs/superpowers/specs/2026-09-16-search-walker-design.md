...
## Best-effort operation attempts

An operation's requiredness belongs to its use in a procedure, not to its operation class. Add a generic operation-attempt value containing a `CoreOperation` and `required: Boolean = false`. The default means “do as much as possible”: attempt the operation, but skip it when a typed rule-impossibility reason prevents it. `required = true` means any validation rejection fails the command. Payments, the authoritative Search draw and track advancement, and a placement or replacement the player already chose are required; a Supply gain is not intrinsically required merely because it uses `AdjustSupply`. “May” text is handled by a player decision, not by skipping a valid attempt. Recover and Forge may still take no relic through explicit procedure branches; they need not manufacture invalid operations to be filtered.

The validator reports reasons against each attempt's staged input state. A separate attempt runner classifies them: an optional attempt skips only when every blocking reason is a rule-impossibility reason. Structural errors, malformed operations, missing sources, inconsistent state, and replay corruption always fail, even on an optional attempt. Validation and filtering happen in operation order because earlier operations can change later legality. The runner returns the operations that actually executed plus skipped reasons; the walker journals only executed operations for replay. Existing strict callers retain their current behavior through the current pipeline API until individually migrated to attempts.
...
Register Dazzle as an automatic walker contribution. It matches only a faceup play of Dazzle, reads the staged post-placement state, and declares discard attempts for Hearth and Order cards at sites in the actor's region in deterministic site/card order. These attempts use the default best-effort policy. The generic staged runner skips a card only for a typed discard-immunity or other rule-impossibility reason; eligible cards are discarded even when another target is immune. An unexpected structural error still rejects the command transaction. Discard operations use existing semantic `Discard` behavior for card orientation and resource returns. The same attempt contract serves future powers with one or many discards; Dazzle contains no special filtering algorithm.
