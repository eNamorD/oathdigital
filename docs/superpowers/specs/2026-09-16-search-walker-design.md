...
`SearchProcedure` is one registered action. Its tree validates the supported Search state, source, actor, Supply cost, and temporary-hand availability; spends Supply; draws the authoritative source order; updates the Visions Drawn track when a world-deck Vision stops the draw; asks the card-selection decision; and embeds `CardPlayProcedure` for the kept card. World deck uses head-as-top; regional discard uses end-as-top. World draws stop at the first Vision, with at most three cards. A one-card draw chooses that card without parking a forced selection.
...
Register Dazzle as an automatic walker contribution. It matches only a faceup play of Dazzle, reads the staged post-placement state, and plans discards of Hearth and Order cards at sites in the actor's region in deterministic site/card order. For each candidate, evaluate current discard permission and immunity before emitting an operation. Immune cards remain; eligible cards are discarded even when another target is immune. Do not treat an executor failure as immunity or swallow it: an unexpected structural error rejects the command transaction. Discard operations use existing semantic `Discard` behavior for card orientation and resource returns.
