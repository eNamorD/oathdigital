...
| People's Favor: Mob | Printed: "When playing a card to a site, you may discard a denizen there first." Holder only. When you play a card to a site you may first discard a card from the site's card list (a denizen, or a ruined edifice). It is enabled through `PlacementRules.siteDiscardFirst`. At a full site, where a play is normally impossible, it makes the play legal. The card goes through the standard discard: facedown to the next region's discard, favor to the suit bank, secrets to you facedown. A ruined edifice is a valid discard and goes back to the edifice deck; an intact edifice is refused as locked (product ruling). `DiscardRestrictions` decide what is discardable. Mob's printed "ACTION: Place favor on the banner" is the generic Place Banner Resource action. Implemented (slice 4b). |
