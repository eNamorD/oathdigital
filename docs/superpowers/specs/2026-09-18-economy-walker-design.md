...
Start gates stay in `build`, because they are facts about state and not costs: the act-phase gate, a pawn site, and `PowerRuntime.requireAudited`. The exile-only and Foundation-profile checks are dropped. The plan decides the fate of the two other checks in the legacy `validateSupportedState`, the bounded warband supply for the actor's force kind and the site-ruler mapping: each survives only if the executor or the tree cannot handle its absence. Supply and resource affordability are not build gates: the transformed `SpendSupply` and `PayCost` own them.
