...
- [ ] **Step 4: implement** Forge's `Partition`, its single-resource no-park path, its `PayCost` translation, the `ForgeRules` affordability gate, and the suit-bank deletions with `ForgeOutcomeMismatch`.
