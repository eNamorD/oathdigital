...
- Raw `OperationExecutor.execute` retains the `OperationShape` guard inside `applyOperation` (double-run with the pipeline is harmless and deliberate this phase).
- `OperationValidator` (class) wraps allowlist + restriction partitions; restriction registry is empty until Phase 5.
