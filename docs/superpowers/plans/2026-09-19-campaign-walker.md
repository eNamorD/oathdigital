...
16. **Tests written for the truncated tree change in Task 11.** Once the attack roll is in the tree, committing force runs the automatic rolls in the same command, so the Task 9 and 10 tests that ended on `WalkerCompleted` end on the sacrifice decision instead, and the suite's `rules` takes permissive test dice (`CampaignFixture.anyDice`). The several-target placement test needs force 5 (four swords tie the four defenders' force, and a tie is a defeat).
