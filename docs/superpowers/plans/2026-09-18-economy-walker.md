...
1. **The payment is a `BuildOps` batch, not a bare `PayCost`.** `PayCost` is a composite, so as a bare child the walker walks its best-effort `Move`s one at a time and an unaffordable payment shrinks to nothing (`required` only bites when `PayCost` is validated whole, inside a `BuildOps` batch, as Forge does). `EconomyTree` therefore puts `PayCost` and `SpendSupply` in one `BuildOps` that is the child of the `<Action>Cost` window. A power that changes or removes the cost rewrites that node, and whatever payment remains is enforced after the fold, which is what the spec asked for. An unaffordable payment is a rejected batch, so the preview drops the option and the start gate rejects a start with nothing playable. (A first version checked `Costs.plan` before the fold; review found that hid the cost window from powers.)
