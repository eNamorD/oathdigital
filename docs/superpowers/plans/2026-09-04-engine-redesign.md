...
3. **Executor = authority; validator pre-flights.** Final ops batch validated before execution; invalid events never recorded. Resource sufficiency = executor concern (atomic batch all-or-nothing). Exception: supply-capped gains ("gain as much as the bank holds") are exact-vs-best-effort per-op semantics (Phase 2).
...
- **Phase 2 — Op vocabulary + cap semantics:** exact vs as-much-as-possible per-op; favor/secrets gains capped at supply; validator pre-flight split for costs.
