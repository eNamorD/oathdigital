...
3. **Executor = authority; validator pre-flights.** Final ops batch validated before execution; invalid events never recorded. Resource sufficiency = executor concern (atomic batch all-or-nothing). Supply-capped gains ("gain as much as the bank holds") clamp at plan time in the effect author (`LimitedResource.clamp`) — there is no per-op best-effort/requireExact executor mode.
