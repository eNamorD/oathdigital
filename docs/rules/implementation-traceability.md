...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (multi-site bandit/player Conquest; accessible 0..maximum force; ordered Outriders and Brass Army attacker plans)** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); Pass targeting [CR pp. 21, 31]; facedown plan reveal and split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | The mandatory pawn-site ruler is recorded as a typed Bandit/player defender, and optional targets must share it. Defense aggregates printed dice and all targeted forces; a defending title holder adds the title die. The registered policy removes every targeted force, kills half a player's aggregate force rounded down, returns survivors to that board, then applies canonical atomic attacker placement while conserving both players' finite warbands. Projection exposes only public defender/force/pool facts; the defender has no controls. Relevant unsupported defender handlers block. Bandit behavior, Pass/canonical validation, empty attack pools, Vow of Peace, Outriders, Brass Army, replay, and local placement remain covered as documented in `architecture/bounded-campaign.md`. | Defender choices/plans, Raid, Empire defenders, and printed replacement/relocation/loss powers remain. |
