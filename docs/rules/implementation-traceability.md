...
|---|---|---|---|---|
| First-game common setup, map population, decks, banks, tracks, player wealth and legacies | **Unimplemented** | [CR pp. 6-7](rules-reference.md#very-first-game-only); changed/randomized by [NF p. 8](new-foundations-delta.md#campaign-structure) | `SetupRules` accepts an already chosen eight-site order and participants; it does not construct this state. Model only: `model/Game.scala`, `model/World.scala`, `model/Cards.scala`. | Foundation/card/component data B1-B5; first-game citizenship policy F8. |
...
| Setup event persistence | **Tested** | Engineering support, not a rulebook statement | `setup/Setup.scala`, `engine/Engine.scala`, and `serialization/EventWire.scala` provide event evolution, atomic journal/replay, versioned wire format and catalog identity checks. Tests: `setup/SetupRulesSuite.scala`, `engine/InMemoryJournalSuite.scala`, `serialization/SetupEventWireSuite.scala`. | Extend the event vocabulary for each future setup step without presenting persistence mechanics as game rules. |

## Turn and phase flow

| Rule area | Status | Normative source / NF change | Implementation and test evidence | Dependencies / unresolved ambiguity |
|---|---|---|---|---|
| Round order and Wake → Act → Rest transitions | **Unimplemented** | [CR p. 17](rules-reference.md#round-and-turn-flow) | `Phase`, `TurnState`, and `GameTracks` exist in `model/Game.scala`; no transition rules advance phase, player, or round. | Action transaction boundary F1; timing/order logging F2. |
| Wake victory checks, Wake powers, Take Wealth | **Unimplemented** | [CR p. 17](rules-reference.md#wake); Take Wealth is new context [NF p. 11](new-foundations-delta.md#map-and-pieces) | Title, Vision, site-token, pawn and used-power state are modeled; no Wake evaluator exists. | Power/component data B1; victory tie rules F7. |
...
| Travel | **Blocked** | [CR p. 21](rules-reference.md#travel---1-to-4-supply-before-modifiers) | Regions and pawn site are modeled; no legality, cost or movement behavior exists. | Site power/card data B1/B5; Pass consent and special target issues F9. |
...
| Rule area | Status | Normative source / NF change | Implementation and test evidence | Dependencies / unresolved ambiguity |
|---|---|---|---|---|
| Durable campaign aggregate: Atlas, lineages, roles, foundations, Reliquary, reserves, oath, era | **Tested** | Durable state inventory [CR pp. 36-41](chronicle.md#durable-state-to-preserve); persistent changes [NF pp. 7, 9, 12](new-foundations-delta.md#campaign-structure) | Aggregate/card-location invariants are tested in `model/DomainValidationSuite.scala`, `model/CardIndexSuite.scala`, `model/PlayerSetupStateSuite.scala`, and Atlas behavior in `model/WorldModelSuite.scala`. This proves storage/invariants, not Chronicle rules. | Serialization of the full aggregate and migration/version policy are not present. |
...
| New map-order vocabulary and Atlas geography | **Tested** | [NF pp. 12, 16](new-foundations-delta.md#empire-and-other-changes); normative CR pp. 9, 45 | `MapState.hinterward/cradleward` and ordered Atlas helpers are tested in `model/WorldModelSuite.scala`. Only ordering/storage mechanics are covered, not setup/Chronicle use. | None for the bounded ordering behavior. |
...
1. **Complete first-game setup through the first Wake.** Use CR pp. 6-7,
   ...
   pawn placement and Take Wealth. This turns today’s tested placement fragment
   into the smallest playable, replayable boundary. First resolve B2-B5 and F4/F8.
