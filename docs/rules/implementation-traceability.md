...
| Rest powers, token return, secret reveal, Supply refresh | **Tested (bounded)** | [CR p. 18](rules-reference.md#rest) | `gameplay/phases/Rest.scala` handles the exile-only first-game profile: controlled denizen/relic resources return, facedown secrets reveal, the tested Exile Supply table refreshes, and current events validate replay facts. Reached reviewed Rest triggers record replay-validated fallback diagnostics. Application, wire, projection, HTTP and frontend suites cover persistence and reload. | Executable component Rest powers and Imperials/Citizens remain deferred (B1/B3); unaudited vocabulary rejects. |
