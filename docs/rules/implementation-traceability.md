...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (single-site bandit Conquest)** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | `Campaign.scala` implements the mandatory pawn-site bandit target, 2-Supply cost, typed force/dice procedure, hollow-sword/skull and shield/doubler scoring, explicit sacrifice and placement, defeat losses, bandit removal/refill, replay validation, persistence, HTTP/auth, and Scala.js interaction. The browser explicitly commits all board warbands; the engine/API retain bounded partial-force authority. Legality and projection share `CampaignRules`. Relevant powers in all advisers, faceup relics, the target, and actor-ruled sites reject regardless of denizen orientation, with site access derived through `SiteRule`. | Player defenders, additional same-ruler sites, partial-force browser UI, battle plans, multi-site allocation, Raid/dispossession/banner/relocation, and displaced-holder ties remain. See `architecture/bounded-campaign.md`. |
...
Campaign expansion should proceed handler-first: add executable battle-plan
coverage, then player defenders and multi-site Conquest before Raid introduces
hidden-card disposal, banners, and relocation.
