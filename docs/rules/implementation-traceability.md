...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (single-site bandit Conquest; first optional attacker plan)** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); facedown plan reveal and split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | `Campaign.scala` implements the authoritative Conquest procedure and pauses before attack randomness for an actor-private plan decision. Exact-ID discovery executes faceup `denizen.vow-of-peace` and offers only explicit skip plus accessible `denizen.outriders` sources. The recorded choice preserves stable source, zero costs, reveal, mechanical result, and physical dice; replay recalculates ignored skull losses. Projection/redaction, strict development/authenticated HTTP, application persistence, event wire, and Scala.js decoding/commands are tested. All other relevant or unknown handlers still reject with source identity; Bag of Siegeworks, Weeping Banner, Peace Envoy, Brass Army, and Mountain/Plains effects remain blocked. | Further attacker plans, paid pool modifiers, defender plans, victory/defeat/end effects, player defenders, multi-site allocation, Raid, and displaced-holder ties remain. See `architecture/bounded-campaign.md`. |
...
Campaign expansion should proceed handler-first: add a typed optional-plan
choice recorded in the existing pre-release event stream, then implement a
small modifier/cost set before player defenders and multi-site Conquest. Raid
continues to wait for hidden-card disposal, banners, and relocation.
