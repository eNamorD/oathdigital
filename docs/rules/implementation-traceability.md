...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (multi-site bandit Conquest; accessible 0..maximum force; ordered Outriders and Brass Army attacker plans)** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); Pass targeting [CR pp. 21, 31]; facedown plan reveal and split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | `Campaign.scala` authoritatively requires the pawn site and accepts optional additional sites ruled by the same bandit defender. Commands, events, pending state, projection, and replay carry canonical target and per-target placement vectors. Victory removes every targeted bandit force through a typed losing-force policy, then atomically applies nonnegative zero/partial/distributed placement while preserving finite warbands. The UI keeps accessible per-site allocation local until confirmation. Refill and Supremacy run once after the whole placement. Pass-blocked, missing, duplicate, reordered, stale, differently ruled, foreign, or excessive facts reject. The Empty Attack Pool permits force zero; Vow of Peace, Outriders, and Brass Army work as documented in `architecture/bounded-campaign.md`. | Further plans, player defenders, Raid, and power-specific replacement/relocation/loss policies remain. |
