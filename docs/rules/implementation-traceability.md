...
| Reusable public board-target selection | **Tested (foundation)** | Interaction support for Travel [CR p. 21](rules-reference.md#travel---1-to-4-supply-before-modifiers), Campaign [CR pp. 22-23](rules-reference.md#campaign---2-supply), and Economy [CR p. 24](rules-reference.md#muster---1-supply) | `GameProjection` and `GameHttpWire` project typed site, site-card, adviser, and relic refs with server-authorized candidates, prompts, cardinality, and details. Scala.js action-first selection implements immediate single choice, cancellable optional modes, persistent multi-choice/confirm, semantic controls, and context invalidation without using `pendingCardDecision`. Setup, Travel, Muster, both Trade modes, strict decode, redaction, keyboard/state behavior, and future adviser/relic hooks are tested. | Campaign must populate its own multi-site candidates and execute no mechanics through this foundation alone. |
...
| After-action fixed-point checks: bandit refill and title transfer | **Unimplemented** | [CR pp. 15-16](rules-reference.md#core-state-and-terminology) | `SiteRule` derives current rule from physical forces, but no shared action-completion boundary or state-based evaluator exists; refill is intentionally not attached to individual actions. | Implement the Campaign action boundary and explicit transaction/fixed-point policy F1. |
...
| Campaign: Conquest/Raid targeting and battle resolution | **Partial foundation** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | `SiteRule` directly derives the defender/ruler from `SiteForces`, resolves Exile lineage to the current player, and supplies tested player-rule, same-ruler and enemy semantics. Public projections expose force kind/count and ruler identity. `PendingProcedure.Campaign` exists; no Campaign transition exists. | Target selection (mandatory origin and optional same-ruler sites), force formation, dice/battle plans, losses, conquest/raid outcomes, RNG/event policy F6, and bandit refill/action boundary F1 remain. |
| Muster | **Tested (bounded)** | [CR p. 24](rules-reference.md#muster---1-supply); variable yield [NF p. 13](new-foundations-delta.md#actions) | `gameplay/actions/Economy.scala` validates an empty denizen at the pawn site, spends 1 Supply/favor, gains 1 + matching faceup advisers subject to the finite 14-warband supply, and records replay-validated v6 events. Commands, scoped projections, HTTP intents, persistence and Scala.js controls share this path; `EconomySuite` covers base/NF yield, limits, rejection, tampering and redaction. | Relevant unimplemented Economy powers reject through typed rule resolution; generic after-action checks remain F1. |
...
|---|---|---|---|---|
| Physical bandits rule empty sites and refill after actions | **Partial** | [NF p. 11](new-foundations-delta.md#map-and-pieces); normative CR p. 15 | Occupied Bandit forces derive `SiteRuler.Bandits` and project as accessible yellow-green “Bandit Warbands”; `SiteForces.Empty` remains explicitly unruled. No refill transition exists because there is no safe general action-completion seam. | Campaign action boundary F1. |
