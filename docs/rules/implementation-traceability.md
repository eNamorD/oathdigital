...
| Rest powers, token return, secret reveal, Supply refresh | **Tested (bounded)** | [CR pp. 15, 17-18](rules-reference.md#rest) | `PlayerResourceSources` centrally discovers adviser denizens, held relics, and deduplicated pawn/ruled site cards. `RestCleanupPlan` consumes it for command/replay/evolution, returns favor by suit and attributable secrets, preserves relic favor, clears only scoped cards, flips facedown board secrets, and refreshes the tested Exile Supply table. Active-player `PlayerSecretSummary` consumes the same source value; inactive summaries require zero owned-card commitments and otherwise fail projection explicitly. Exact codecs and accessible Scala.js labels display available/`totalSecrets`. Tests cover overlap, out-of-scope cards, invalid inactive attribution, replay tampering, HSQL reopen and codec equality. | Executable component Rest powers and Imperials/Citizens remain deferred (B1/B3); unaudited vocabulary rejects. Site-card commitment attribution is limited to the active player's printed access scope. |
...
| Core minor actions: adviser play/discard, relic peek, warband move, all-Exile negotiation | **Bounded implementation** | [CR p. 27 and pp. 32, 34-35](rules-reference.md#minor-actions); negotiation changes [NF p. 15](new-foundations-delta.md#actions) | `gameplay/actions/MinorActions.scala`, `gameplay/actions/Negotiation.scala`, current event records, owner-scoped knowledge and participant projections. Facedown-adviser play has one Search-derived draft control, reuses authoritative modifier preview/order validation, and submits one typed discard/faceup resolution. Site relic peeks persist private ordered knowledge and render as temporarily revealed inspectable backs only for the knowing viewer. Reviewed reached When Played triggers record fallback diagnostics. Negotiation persists authored favor/relic/disclosure terms, resets consent on revision, and applies unanimous deals atomically. | Executable Action/When Played powers B1; Citizenship/Grand Scepter, self-exile, Imperial transfer, remote and component-modified Negotiation remain deferred. Unaudited vocabulary rejects. |
