...
| Wake victory checks, Wake powers, Take Wealth | **Tested (bounded)** | [CR p. 17](rules-reference.md#wake); power identity/timing CR pp. 28, 31; Take Wealth site context [NF p. 11](new-foundations-delta.md#map-and-pieces) | `FirstTurnWake` transfers one loose favor/secret and records stable timing/source/power references. Enemy/resource/per-site-use legality and private controls share the typed query pipeline in `gameplay/RuleResolution.scala`. Mandatory title/Vision states return typed unsupported violations. Tests: `gameplay/FirstTurnWakeSuite.scala`, mixed v2/v3 wire and application suites. | Generic powers, River movement and victory resolution remain deferred. Optional unsupported Wake powers do not block End Wake. |
