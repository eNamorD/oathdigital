...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (multi-site bandit/player Conquest; accessible 0..maximum force; ordered Outriders and Brass Army attacker plans)** | [CR pp. 22-23, 29](rules-reference.md#campaign---2-supply); Pass targeting [CR pp. 21, 31]; facedown plan reveal and split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | The mandatory pawn-site ruler is recorded as a typed Bandit/player defender, and optional targets must share it. Defense aggregates printed dice and targeted forces. Since the title is an optional defender plan (Oathkeeper +1 die, Usurper +2), titled defenders explicitly block until that choice window exists. Registered replay-validated policy results dispose of both defeated site forces and defeated attacker force; default player losses kill half rounded down and return survivors, while dormant preserve/relocate forms are mechanically executable. Projection exposes only public defender/force/pool facts; the defender has no controls. Bandit behavior, Pass/canonical validation, empty attack pools, attacker-plan support, replay, and local placement remain covered in `architecture/bounded-campaign.md`. | Defender choices/plans, Raid, Empire defenders, and printed alternate loss powers remain. |
