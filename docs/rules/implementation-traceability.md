...
| Campaign: Conquest/Raid targeting and battle resolution | **Tested (single-site bandit Conquest; typed power boundary)** | [CR pp. 22-23](rules-reference.md#campaign---2-supply); split/ordering/discard changes [NF p. 13](new-foundations-delta.md#actions) | `Campaign.scala` implements the existing authoritative Conquest procedure. `RuleResolution.scala` names all eight Campaign timing windows. Exact-ID discovery orders stable sources and handlers, executes faceup `denizen.vow-of-peace`, classifies audited bandit-irrelevant powers, and rejects blocked or unknown relevant handlers with source identity. Facedown advisers remain discoverable but passive text is inactive; Bag of Siegeworks, Weeping Banner, Peace Envoy, and Mountain/Plains effects reject conservatively. | No optional printed battle plan is auto-selected: typed player selection, costs/modifiers, defender plans, victory/defeat/end effects, player defenders, multi-site allocation, Raid, and displaced-holder ties remain. See `architecture/bounded-campaign.md`. |
