...
| Rest powers, token return, secret reveal, Supply refresh | **Tested (bounded)** | [CR pp. 15, 17-18](rules-reference.md#rest) | `RestCleanupPlan` is the pure command/replay/evolution source for the resting player's adviser denizens, held relic secrets, pawn-site cards even when unruled, and deduplicated ruled-site denizens/edifices. It returns favor by suit, returns attributable secrets, preserves relic favor, clears only scoped cards, flips all facedown board secrets, and refreshes the tested Exile Supply table. `PlayerSecretSummary` derives available/facedown/committed/`totalSecrets`; exact projections and accessible Scala.js labels display available/total without identity leakage. Tests cover overlap, out-of-scope cards, replay tampering, HSQL reopen and codec equality. | Executable component Rest powers and Imperials/Citizens remain deferred (B1/B3); unaudited vocabulary rejects. Site-card commitment attribution is limited to the active player's printed access scope. |
