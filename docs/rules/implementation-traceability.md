...
| Wake victory checks, Wake powers, Take Wealth | **Tested (bounded)** | [CR p. 17](rules-reference.md#wake); power identity/timing CR pp. 28, 31; Take Wealth site context [NF p. 11](new-foundations-delta.md#map-and-pieces) | `gameplay/phases/Wake.scala` transfers one loose favor/secret and records stable timing/source/power references. Enemy/resource/per-site-use legality and private controls live in the Wake-owned `gameplay/phases/TakeWealthRules.scala` (relocated from `RuleResolution.scala` in the terrain migration; `usedPowers` tracking stays outside the power framework). `gameplay/StateBasedEvaluation.scala` handles the bounded Oathkeeper, Usurper, and Vision paths. Tests cover Wake, state-based evaluation, wire, and application behavior. | Generic powers, River movement, Imperials, and altered Foundations remain deferred. Optional unsupported Wake powers do not block End Wake. |
...
| Rest powers, token return, secret reveal, Supply refresh | **Tested (bounded)** | [CR pp. 15, 17-18](rules-reference.md#rest) | `RestPowerIntegration` resolves precise ordered pre-cleanup hooks while `RestPowers` remains classification-only. League Treaty's registered typed handler owns its concrete decision payload, events, validation, and effects: it inspects its faceup site source, current ruler, Rest actor, regional favor-bearing denizens/edifices/site relic slots, and legal banks; the off-turn ruler may decline or atomically move bounded favor before ordinary cleanup resumes. Facedown relic identities remain redacted. `PlayerResourceSources` and `RestCleanupPlan` then perform the existing token return, secret reveal, Supply refresh, and transition. Typed commands/events, replay, scoped projections, HTTP mapping, and Scala.js controls cover ownership, tamper rejection, ordering, redaction, and continuation. | Remaining component Rest powers and Imperials/Citizens remain deferred (B1/B3); unaudited vocabulary rejects. Site-card commitment attribution is limited to the active player's printed access scope. |
...
| Travel | **Tested (bounded)** | [CR p. 21](rules-reference.md#travel---1-to-4-supply-before-modifiers); site powers CR p. 31 and NF p. 11; consent timing CR p. 43 | `gameplay/actions/Travel.scala` implements first-turn exile-only normal Travel with the region matrix and Supply debit. Cost and pass legality run on typed TravelCost window powers under `gameplay/powers/travel/` (`TravelCostWindow.scala`): terrain site powers declare typed cost facts, the window fold computes route topology, the suppression registry expresses "Coast ignores Island/Mountain/Pass on a coast route", and Narrow Pass is a power whose restriction body Travel legality evaluates against a simulated pawn move. Command, deterministic replay, private legal-destination projection, and destination-selection UI share resolution. Tests cover the fold, suppression, pass restriction, engine, wire, application reload, projection, and frontend behavior. | River, consent workflow, generic card/relic/edifice/legacy/Foundation modifiers, later turns and Rest remain deferred. Reviewed optional handlers are omitted from selection; unaudited vocabulary rejects. |
