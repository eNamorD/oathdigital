...
private[frontend] final class TestBrowser(search: String = "") {
  private val browser: js.Dynamic = new js.Function("search", """return (() => {
    ...
    // jsdom has no ResizeObserver; the map viewport only needs it to exist.
    const stubbedObserver = !window.ResizeObserver;
    if (stubbedObserver) define(window, 'ResizeObserver', { value: class {
    ...
    return { mount, urls, copied,
      ...
      close() {
        ...
        if (stubbedObserver) delete window.ResizeObserver;
